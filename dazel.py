...
class DockerInstance:
    ...
    def from_config(cls):
        ...
        return DockerInstance(
                ...
                dazel_run_file=config.get("DAZEL_RUN_FILE",
                                          os.path.join(DockerInstance._find_workspace_directory(), DAZEL_RUN_FILE)),
    ...
    def _run_container(self):
        ...
        command += "%s run --tmpfs=/tmp --tmpfs=/var --sysctl=net.ipv6.conf.all.disable_ipv6=0 --read-only -id --name=%s %s %s %s %s %s %s %s%s %s" % (
            self.docker_command,
            self.instance_name,
            "--privileged" if self.docker_run_privileged else "",
            ("--user=%s" % self.user
